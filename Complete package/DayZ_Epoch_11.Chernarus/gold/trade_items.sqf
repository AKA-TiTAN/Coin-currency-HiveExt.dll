...
_emptySlots = [_player] call BIS_fnc_invSlotsEmpty;
...
for "_x" from 1 to _total_trades do {
	...
	if (_finished) then {
		_player_wealth = _player getVariable ["wealth",0];
		if (_player_wealth >= _price) then {
			/*PVDZE_obj_Trade = [_player,_traderID,0];
			publicVariableServer  "PVDZE_obj_Trade";

			waitUntil {!isNil "dayzTradeResult"};
			if (dayzTradeResult == "PASS") then {*/
				_player setVariable["wealth",(_player_wealth - _price),true];
...
for "_x" from 1 to _total_trades do {
	...
	if (_finished) then {
		...
		_removed = ([_player,_name,1] call BIS_fnc_invRemove);
		

		if (_removed > 0) then {
			_player_wealth = _player getVariable ["wealth",0];
			/*PVDZE_obj_Trade = [_player,_traderID,1];
			publicVariableServer  "PVDZE_obj_Trade";
			waitUntil {!isNil "dayzTradeResult"};
			if(dayzTradeResult == "PASS") then {*/
				_player setVariable["wealth",(_player_wealth + _price),true];
