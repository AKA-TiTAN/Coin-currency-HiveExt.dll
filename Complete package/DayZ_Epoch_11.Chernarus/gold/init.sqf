...
LimitOnBank = false;
MaxBankMoney = 10;
DonatorList = ["111111111","111111111"];
...
BankDialogUpdateAmounts = {
	ctrlSetText [BankDialogPlayerBalance, format["%1 %2", (player getVariable ['wealth', 0]), "Coins"]];
	ctrlSetText [BankDialogBankBalance, format["%1 %2", (player getVariable ['bank', 0]), "Coins"]];
};

GivePlayerDialogAmounts = {
	ctrlSetText [GivePlayerDialogPlayerBalance, format["%1 %2", (player getVariable ['wealth', 0]), "Coins"]];
...
BankDialogDepositAmount = {
	...
	if(   (getPlayerUID player in DonatorList )  && ((_bank + _amount ) <  MaxDonatorBankMoney)) then{ 
