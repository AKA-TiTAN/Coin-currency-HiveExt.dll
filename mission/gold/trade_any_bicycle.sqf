...
if(_buy_o_sell == "buy") then {
	//_qty = {_x == _part_in} count magazines player;	
		_qty = player getVariable ["wealth",0]; // get your money variable	
		...
		if (_qty >= _qty_in) then {
			...
			if(dayzTradeResult == "PASS") then {
				...
				if(_buy_o_sell == "buy") then {	
					...
					_oudaantal = player getVariable ["wealth",0];
					...
					player setVariable ["wealth", _qtychange , true];	
					_newM = player getVariable ["wealth",0];
					//_removed = ([player,_part_in,_qty_in] call BIS_fnc_invRemove);
					
					_removed = _qty - _newM; // 
					
						systemChat format ['You have paid %1 Coins. %2 incoming!',_removed,_part_out];
					...
					if(local _obj) then {
						...
						if(_okToSell) then {
							...
							if(!isNull _obj and alive _obj) then {
								...
								_myMoney = player getVariable ["wealth",0];							
								_myMoney = _myMoney + _qty_out;								
								player setVariable ["wealth", _myMoney , true];
